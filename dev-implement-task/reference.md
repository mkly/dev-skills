...
you need exact behavior. All scripts are bundled with the installed
dev-implement-task skill, not with the target repo. In command examples,
`/path/to/dev-implement-task-skill`
...
Task creation belongs to the sibling `dev-create-tasks` skill. Its
`dct-create.sh` atomically imports a complete pending task with a pre-generated
...
### Create a task (delegated to dev-create-tasks)

```sh
uuid="$(/path/to/dev-create-tasks-skill/scripts/dct-create.sh \
  --goal <goal-slug> --loop-id <uuid> --description "implement X" \
  --acceptance "<observable proof that X is done>")"
```

The helper returns the exact UUID on stdout and leaves the task pending,
unstarted, and unassigned. It derives the project from the lowercase GitHub
`origin` basename and records `repo-id:`, `goal:`, `loop-id:`, and
`loop-round:`. All initial tasks reuse the same goal and loop UUID. Use
`--from-task <uuid>` for follow-up identity inheritance and `--json` when a
downstream task needs the predicted review branch. Pass both
`--depends <uuid>` and `--input <review-branch>` when code ancestry matters.
...
the unconfigured-sync result described by dev-create-tasks.
...
1. Derive the current repository project and repo ID from its GitHub origin;
   reject foreign, malformed, acceptance-free, or blocked tasks before mutation.
...
3. `flock -w 10` on `$DEV_LOOP_STATE_DIR/locks/select.lock` (fd 9) — a true
   ...
   time.
4. Inside the lock, write `assignee:${DEV_LOOP_OWNER}#${AGENT_PID}` (or
   `#<pid>-<random>` when `AGENT_PID` is unset), then read it back via
   `task export | jq`; if the exact value is not still present, exit `10`.
   Display and ownership checks strip the `#...` nonce suffix, but re-claim
...
```sh
/path/to/dev-implement-task-skill/scripts/dl-claim.sh <uuid> --steal-after 4h      # specific stale task
/path/to/dev-implement-task-skill/scripts/dl-claim.sh --steal-after 4h             # auto-pick incl. stale actives
```
...
warmup session remains live. Implementation may proceed only after exit `0`, a handle
...
After `dev-complete-task/scripts/dlc-done.sh` or `dl-release.sh`, a still-live
lease is parked for this repository instead of being stopped. The next
`dl-box.sh` atomically claims it,
...
```sh
/path/to/dev-implement-task-skill/scripts/dl-run.sh "$uuid" -- bash -lc 'apt-get update && make'   # syncs worktree up
/path/to/dev-implement-task-skill/scripts/dl-run.sh "$uuid" --compact -- bash -lc 'make test'      # compact with in-box RTK, raw fallback
/path/to/dev-implement-task-skill/scripts/dl-run.sh "$uuid" --no-sync -- bash -lc 'make test'      # fast re-run, skip upload
...
Before returning the review-ready task, add its `summary:` annotation and run
`task rc.confirmation=no sync`. Tolerate only the explicitly unconfigured sync
result; other failures are environment blockers.
...
`dl-release.sh`, `dev-complete-task/scripts/dlc-done.sh`, the printed
`crabbox stop` hint, or
`git worktree prune`.
...
its `branch=` annotation across **all** tasks so active and legacy-completed
producers are both visible. The candidate set is every `branch=`
...
dev-create-tasks helper records the two-part link for a consumer: `--depends
...
annotation added before returning to `dev-complete-task` captures what changed,
why, and which checks passed. Completion preserves it when it marks the task
done, making each finished task self-describing.

Because state is durable here and not in the agent's head, resume interrupted
...
| `dl-run.sh`/`dl-merge-back.sh` exit `20` "recorded worktree is missing" | The worktree dir was deleted/pruned out from under the task. Re-run `/path/to/dev-implement-task-skill/scripts/dl-box.sh <uuid>` to recreate it on the same scratch branch, then retry. |
...
| Two agents, same Unix user, claims collide on identity | Launch via `loop`, which mints a distinct owner per worker; otherwise export a distinct, stable literal `DEV_LOOP_OWNER` per agent before setup (never a $$-derived value). |
| A worker picks up a task another worker is implementing | Both share a `DEV_LOOP_OWNER` — check `loop`'s startup line and `dl-status.sh`. Agents launched outside `loop` fall back to the shared state file owner. |
