...
TEST_RUN="$ROOT/scripts/dlr-test.sh"
TMP="$(mktemp -d "${TMPDIR:-/tmp}/dlr-test-compact-test.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
export DLR_WORKTREE_DIR="$TMP/worktrees"
...
printf 'ok: dlr-test compact mode and fallback tests\n'
