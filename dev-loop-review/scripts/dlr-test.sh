#!/usr/bin/env bash
# dlr-test.sh — Phase 2 helper: run a check (tests/build/lint) against a
...
# dlr-merge.sh's audit annotation (see SKILL.md operating rules). So this
# script's lease is keyed on the BRANCH itself via a deterministic slug —
...
[ "${#CMD[@]}" -gt 0 ] || { usage; dlr_die "$DLR_PRECOND" "no command given (expected '<branch> -- <cmd>')"; }

dlr_require git jq crabbox
dlr_in_git_repo
git rev-parse --verify --quiet "refs/heads/${BRANCH}" >/dev/null \
  || dlr_die "$DLR_MISSING" "review branch '${BRANCH}' is not present locally"

slug="$(dlr_slug_for_branch "$BRANCH")"
wt="$(dlr_worktree_dir_for "$slug")"
label="dev-loop-review/${BRANCH}"
...
ensure_worktree() {
  ...
  if git -C "$path" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    if [ "$cur_head" = "$branch_head" ]; then
      dlr_log "reusing test worktree for ${branch}: $path"
      return 0
    fi
    dlr_log "test worktree at $path is stale (not at ${branch}'s tip); recreating"
    if [ "$DRY" -eq 0 ]; then
      git worktree remove --force "$path" >/dev/null 2>&1 || rm -rf "$path"
    fi
  fi
  if [ "$DRY" -eq 1 ]; then
    dlr_log "DRY-RUN: git worktree add $path $branch"
    return 0
  fi
  git worktree prune >/dev/null 2>&1 || true
  mkdir -p "$(dirname "$path")"
  git worktree add "$path" "$branch" >&2 \
    || dlr_die "$DLR_PRECOND" "could not create worktree at $path for branch $branch (checked out elsewhere?)"
}
...
if box_alive "$slug"; then
  handle="$slug"
  dlr_log "reusing live box for ${BRANCH}: $handle"
else
  mapfile -t incus_flags < <(dlr_crabbox_incus_flags)
  if [ "$DRY" -eq 1 ]; then
    dlr_log "DRY-RUN: crabbox warmup -provider $CRABBOX_PROVIDER ${incus_flags[*]:-} -slug $slug -ttl $DLR_TEST_TTL"
    handle="$slug"
  else
    warmout="$(mktemp "${TMPDIR:-/tmp}/dlr-warm.XXXXXX")"
    trap 'rm -f "$warmout"' EXIT
    dlr_log "warming box for ${BRANCH} (slug=$slug ttl=$DLR_TEST_TTL)"
    # Warm from the test worktree so crabbox's repoRoot matches where `crabbox
    # run` below will cd, same reasoning as dev-loop's dl-box.sh.
    if ! ( cd "$wt" && crabbox warmup -provider "$CRABBOX_PROVIDER" \
          ${incus_flags[@]+"${incus_flags[@]}"} \
          -slug "$slug" -ttl "$DLR_TEST_TTL" ) >"$warmout" 2>&1; then
      cat "$warmout" >&2
      dlr_die "$DLR_PRECOND" "crabbox warmup failed (see output above)"
    fi
    cat "$warmout" >&2
    handle=""
    ...
    [ -n "$handle" ] || dlr_die "$DLR_PRECOND" "could not resolve a live box handle after warmup"
  fi
fi
...
if [ "$KEEP_BOX" -eq 0 ] && [ "$DRY" -eq 0 ]; then
  crabbox stop -provider "$CRABBOX_PROVIDER" -id "$handle" >/dev/null 2>&1 \
    || dlr_warn "could not stop box $handle (may already be gone)"
fi
